...
class rmfAudio_L1_L2_tests(utHelperClass):
    """
    Test class to run the L1 tests on the device.
    This class uses the `rmfAudioClasses` to interact with the device
    """
    def __init__(self, log:logModule=None):
        ...
        testSetupPath = os.path.join(dir_path, "rmfAudio_L1_L2_testSetup.yml")
    ...
    def testFunction(self):
        ...
        for testsuite in testsuites:
            ...
            testrmfaudio = rmfAudioClass(self.moduleConfigProfileFile, self.hal_session, testsuite_name, self.targetWorkspace)
            ...
            if len(test_cases) == 1 and test_cases[0] == "all":
                ...
                for test_case in testsuite.get("test_cases"):
                    ...
                    result = testrmfaudio.runTest(test_case)
                    finalresult &= result
...
if __name__ == '__main__':
    ...
    test = rmfAudio_L1_L2_tests(summeryLog)
    test.run(False)
